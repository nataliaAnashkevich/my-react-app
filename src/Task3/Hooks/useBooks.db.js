...
const useBooks = () => {
    ...
    const updateBook = (editedItem, bookName, bookPrice) => {
        ...
        // через map + получать отдельно id, bookName, bookPrice - заменить editedItem
        // или через только editedItem - чуть менее понятно
        // setBookList - сначала передавать предыдущее состояние
        // можно отдельные длинные функции можно вынести отдельно: которая передается в setBookList (и тп), setError (для формирования ошибки)
